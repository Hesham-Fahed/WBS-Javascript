...
// Wird diese PHP-Seite über einen AJAX-Request angesteuert oder direkt aufgerufen
$ajax = isset($_SERVER['HTTP_X_REQUESTED_WITH']) &&
        strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest';
$req_term = isset($_REQUEST['term']) ? $_REQUEST['term'] : '';
// WENN keine AJAX-Anfrage vorhanden, erselle ein Formular im HTML-Body
if (!$ajax) {
?>
  <!DOCTYPE HTML>
  <html lang="en">
...
// if(isset($_REQUEST['term']) && $_REQUEST['term'] !== "") {
  ...
  foreach ($entries as $term => $entry) {
      if (stripos($term, strtoupper($_REQUEST['term'])) !== FALSE) {
